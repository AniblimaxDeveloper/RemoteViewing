...
public class MainActivity extends Activity {
    static final String SERVER_URL="ws://192.168.1.10:8080"; // Set to your server
    ...
    void connect(){String c=code.getText().toString().trim(); if(!c.matches("\\d{6}")){status.setText("Masukkan Device ID 6 digit");return;} OkHttpClient client=new OkHttpClient(); ws=client.newWebSocket(new Request.Builder().url(SERVER_URL).build(),new WebSocketListener(){
        ...
        @Override public void onMessage(WebSocket s,String text){try{JSONObject j=new JSONObject(text);String t=j.getString("type"); status.post(()->status.setText(t.equals("SESSION_APPROVED") ? "Sesi disetujui" : t));}catch(Exception ignored){}}
